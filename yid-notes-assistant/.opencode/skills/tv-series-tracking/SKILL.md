...
description: Use when the user wants to save, update, review, or check TV series in their notes system, especially for watch progress, released seasons, and newly released seasons found via internet search.
---
...
Maintain the user's TV series collection as a compact database-like table inside the notes system.
...
Do not store tracked TV series as random separate notes by default.

Instead, keep a central collection note in a logical media location, usually under a TV or media category, as a structured Markdown table.

Separate per-series notes are allowed and often useful when a series needs a short description, extra notes, or richer context than should live inside the table.

Use the table as the compact tracking layer, and use separate per-series notes when there is enough content to justify them.

If the tracking note does not exist yet, create it during the first add operation.

## First-Time Setup

When the user adds the first tracked TV series and no collection file exists yet:

1. Choose a logical location in the media section of the notes system.
2. Create a single collection note for tracked TV series.
3. Add a short title and, if useful, one brief sentence explaining the purpose of the table.
4. Initialize the table with the chosen column layout.
5. Add the first series row immediately.
6. Update `TOC.md` so the new collection note is linked from the relevant media location.

Prefer a stable filename and keep it descriptive, for example something like `tv-series-tracker.md` or `tracked-tv-series.md`, 
adjusted to the actual folder naming style already used in the notes system.
...
This schema is recommended, not rigid. Keep the same overall ideas, but adapt the exact columns if the collection needs 
a slightly different shape in practice.
...
- `Watched up to`: the last season the user has watched. If none, store an explicit empty-state value such as `none` or 
`0`, and use it consistently within the table.
...
Longer notes, short descriptions, and richer commentary should usually live in a separate per-series note rather than in
the table itself.

## Flexibility And Reuse

This skill is written for TV series, but the underlying pattern is broader: a media collection can be stored as a 
central Markdown table with lightweight internet-enriched metadata and user progress fields.

If the user later wants a similar collection for another media type, reuse the same approach:

- keep one central collection note
- prefer a compact readable table
- separate user-progress fields from external factual metadata
- choose columns that fit the domain instead of copying the TV schema blindly

For TV series specifically, keep the recommended schema above unless there is a clear reason to adapt it.

## Adding A Series

When the user asks to save or add a TV series:

1. Find or create the central TV series tracking note in the appropriate place in the notes structure.
...
7. Update `TOC.md` if the collection note or its location is new.
...
If the series has a separate note, update that note when the user provides information that belongs there rather than in
the tracking table.
...
- If sources conflict, prefer the most authoritative and current information. If the ambiguity is worth preserving, put 
  it in the separate per-series note rather than bloating the table.
...
- If the user says they finished all currently available seasons but the series is not over, keep `User status` 
meaningful for the user's workflow and rely on `Watched up to` plus `Latest released` to show that they are caught up.
